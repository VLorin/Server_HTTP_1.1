...
int main(int argc, char *argv[]) {
	...
	while ( 1 ) {
		...
		//vérification sémantique : Paul 
			
			
		//traitement request-target : Bastien
			char *req = requestTarget(root,requete->clientId);
			
			if(req != -1){ //request-target valide, sinon connexion déjà fermée
			
			//traitement de la réponse : Vincent			


				if(fin_connexion(root) == -1){ //le header "connection" vaut "close"
		...
		//syntaxe invalide -> Erreur 400 : Bad Request
