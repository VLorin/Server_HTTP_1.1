...
int conformite_methode(_Token* root, int Id){
	...
	if((compare_chaine(noeud->value,noeud->len,"PUT",3)!=0) && (compare_chaine(noeud->value,noeud->len,"DELETE",6)!=0) && (compare_chaine(noeud->value,noeud->len,"OPTIONS",7)!=0) && (compare_chaine(noeud->value,noeud->len,"TRACE",5)!=0) && (compare_chaine(noeud->value,noeud->len,"CONNECT",7)!=0) ){
		writeDirectClient(Id,ERROR400,strlen(ERROR400));
		endWriteDirectClient(Id);
		requestShutdownSocket(Id);
		return 0;
...
int conformite(_Token *root, int Id){
	if(unicite_header(root) == 1 && conformite_version(root,Id) == 1 && conformite_methode(root,Id) == 1) return 1;
	else return -1;
}
